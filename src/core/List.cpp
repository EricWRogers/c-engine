...
namespace List {
    ...
    void Resize(void* _refList, unsigned int _capacity, size_t _elementSize) {
        printf("Resize %p\n", _refList);
        size_t* elementSize = ((size_t*)(*((void**)_refList))) - 1;
        uint32_t* count = ((uint32_t*)elementSize) - 1;
        uint32_t* capacity = count - 1;
    
        capacity = (uint32_t*)realloc((void*)capacity, sizeof(size_t) + sizeof(uint32_t) + sizeof(uint32_t) + (_capacity * _elementSize));
